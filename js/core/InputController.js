...
export class InputController {
  constructor(onNumberKey, onEmp, onCycleStation, onJump, onDesignate) {
    this.onNumberKey = onNumberKey;
    this.onEmp = onEmp;
    this.onCycleStation = onCycleStation;
    this.onJump = onJump;
    // X : cible prioritaire de l'artillerie. Touche dédiée parce que les chiffres
    // sont les commandes du poste et le clic droit sert déjà à l'anneau d'énergie.
  ...
  _key(e, isDown) {
    switch (e.code) {
      ...
      case 'KeyX': if (isDown) this.onDesignate?.(); break;                      // cible prioritaire
      default:
